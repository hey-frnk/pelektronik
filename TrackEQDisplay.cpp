...
uint8_t TrackEQDisplay::getType() {
  printf("TrackDisplay: Type: %hhu\n", this->type);
  return this->type;
}

void TrackEQDisplay::show() {
  printf("TrackDisplay: Show Method of TrackDisplay Called! Volume: %hhu\n", this->volume);
}

void TrackEQDisplay::setTrackInfo(char *trackName, char *artistName, char *albumName, uint32_t length){
...
void TrackEQDisplay::changeVolume(uint8_t volume){
  this->volume = volume;
  printf("TrackDisplay: Volume change to: %hhu\n", this->volume);
}

void TrackEQDisplay::changeMode(uint8_t mode){
  this->mode = mode;
  printf("TrackDisplay: Mode change to: %hhu\n", this->mode);
}

void TrackEQDisplay::changeStatus(uint8_t status){
  this->status = status;
